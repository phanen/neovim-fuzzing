...
safe(vim.api.nvim__redraw, {["cursor"]=true,["win"]=0})
end
do  -- round 2
safe(vim.cmd, "redraw")
safe(vim.api.nvim_input, "iipax<Esc>udpd<Esc>dupa<CR>ud<Esc><CR>iupxpuaa<Esc>dxixxaxxu<CR><Esc>id<Esc><CR><CR><CR><CR><CR><CR><CR><CR>xpxpa<Esc>ipppdxdi<Esc>iaxp<CR><CR>d<Esc>iip<CR>di<Esc>udddx<CR>a<CR>uu<Esc>p<CR>pxpuuaau<Esc>puxu<CR><CR>xp<Esc>d<Esc>ad<CR>x<Esc>xp<Esc>dipua<Esc>p<CR>i<Esc><CR><CR>a<Esc>a<Esc><Esc>dia<CR><CR><Esc>idixa<Esc><CR>pxxd<Esc>x<Esc>uxp<CR><Esc><Esc>dp<Esc><Esc>x<CR>adpp<Esc>a<Esc>d<Esc><CR><CR>xxi<Esc><Esc>xpa<Esc>uiupuxup<Esc>piaa<Esc>adpxd<CR>uuiiauiuaa<Esc>uupuidup<CR>ud<Esc><CR><CR><Esc><Esc><Esc>dx<Esc>p<CR>x<Esc>uaiipax<Esc>udpd<Esc>dupa<CR>ud<Esc><CR>iupxpuaa<Esc>dxixxaxxu<CR><Esc>id<Esc><CR><CR><CR><CR><CR><CR><CR><CR>xpxpa<Esc>ipppdxdi<Esc>iaxp<CR><CR>d<Esc>iip<CR>di<Esc>udddx<CR>a<CR>uu<Esc>p<CR>pxpuuaau<Esc>puxu<CR><CR>xp<Esc>d<Esc>ad<CR>x<Esc>xp<Esc>dipua<Esc>p<CR>i<Esc><CR><CR>a<Esc>a<Esc>")
end
do  -- round 3
safe(vim.api.nvim_list_bufs)
end
do  -- round 4
safe(vim.api.nvim_input, "vwxyz{|}~GHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~CDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~YZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~<C-w>|./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~9:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~fghijklmnopqrstuvwxyz{|}~jklmnopqrstuvwxyz{|}~DEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~<M-!>š")
end
do  -- round 5
safe(vim.api.nvim_list_bufs)
...
do  -- round 8
safe(vim.api.nvim_list_wins)
safe(vim.api.nvim_paste, "\0\2\3\6\21\22\23\24\27\127,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~VWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~", false, -1)
end
do  -- round 9
safe(vim.api.nvim_list_bufs)
end
...
do  -- round 13
safe(vim.api.nvim_eval, "argv()")
end
do  -- round 15
safe(vim.api.nvim_echo, {[1]={[1]="9:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~`abcdefghijklmnopqrstuvwxyz{|}~GHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~IJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~ !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~"}}, true, {["kind"]="search"})
safe(vim.api.nvim_set_option_value, "guicursor", "", {})
...
do  -- round 16
safe(vim.api.nvim_list_bufs)
safe(vim.api.nvim_exec_autocmds, "BufEnter", {["pattern"]="*.lua"})
end
do  -- round 17
safe(vim.api.nvim_list_wins)
end
...
safe(vim.api.nvim_open_win, 3, false, {["noautocmd"]=false,["focusable"]=false,["relative"]="win",["zindex"]=511,["width"]=154,["height"]=3,["row"]=10,["col"]=47})
safe(vim.api.nvim_list_tabpages)
safe(vim.cmd, "only")
safe(vim.cmd, "only")
safe(vim.cmd, "only")
...
do  -- round 21
safe(vim.api.nvim_list_wins)
safe(vim.api.nvim_input, "<M-ýº–¸‡—>")
end
do  -- round 22
safe(vim.api.nvim_list_bufs)
end
do  -- round 23
safe(vim.api.nvim_set_option_value, "signcolumn", "auto:1-9", {})
end
do  -- round 24
safe(vim.api.nvim_list_wins)
end
do  -- round 25
safe(vim.api.nvim_create_buf, false, true)
safe(vim.api.nvim_open_win, 4, true, {["noautocmd"]=false,["focusable"]=false,["row"]=47,["relative"]="win",["zindex"]=187,["width"]=146,["height"]=40,["hide"]=true,["col"]=-12})
safe(vim.cmd, "tabnew")
safe(vim.cmd, "redrawstatus")
...
os.exit(0)
-- 
-- Source crash: afl-findings/daily-2026-08-22/default/crashes/id:000001,sig:09,src:000006,time:3130642,execs:8506,op:trim,rep:4 (992 bytes)
